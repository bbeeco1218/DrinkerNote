...
abstract class MySwipeHelper_a extends ItemTouchHelper.SimpleCallback {

    int buttonWidth;
    private RecyclerView recyclerView;
    private List<MySwipeHelper_a.MyButton> buttonList;
    private GestureDetector gestureDetector;
    private int swipePosition=-1;
    private float swipeThreshold = 0.5f;
    private Map<Integer,List<MySwipeHelper_a.MyButton>> buttonBuffer;
    private Queue<Integer> removerQueue;
    private  GestureDetector.SimpleOnGestureListener gestureListener= new GestureDetector.SimpleOnGestureListener(){

        @Override
        public boolean onSingleTapUp(MotionEvent e) {
            for(MySwipeHelper_a.MyButton button:buttonList){
    ...
    public MySwipeHelper_a(Context context, RecyclerView recyclerView, int buttonWidth) {
    ...
    public class MyButton{
        ...
        private MyButtonClickListener_a listener;
        private Context context;
        private Resources resources;

        public MyButton(Context context, String text,  int textSize,int imageResId, int color, MyButtonClickListener_a listener) {
            this.text = text;
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            if(dX<0){
                List<MySwipeHelper_a.MyButton> buffer= new ArrayList<>();
    ...
    private void drawButton(Canvas c, View itemView, List<MySwipeHelper_a.MyButton> buffer, int pos, float translationX) {
        float right = itemView.getRight();
        float dButtonWidth = -1*translationX / buffer.size();
        for(MySwipeHelper_a.MyButton button:buffer){
    ...
    public abstract void instantiatrMyButton(RecyclerView.ViewHolder viewHolder, List<MySwipeHelper_a.MyButton> buffer) ;
}